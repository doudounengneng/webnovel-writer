...
program
  ...
  .option('--provider <provider>', 'LLM provider: openai, anthropic, or mock', 'mock')
  .option('--model <model>', 'Model name (default: gpt-4o for openai)')
  .option('--api-key <key>', 'API key (or set LLM_API_KEY env var)')
  .option('--base-url <url>', 'Custom API base URL')
  .option('--use-env', 'Use environment variables for LLM configuration (recommended)')
  .option('-d, --dir <path>', 'Project directory', '.')
